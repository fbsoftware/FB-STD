...
editor.renderFromState = function () {
  ...
  Object.values(editor.state.sections).forEach(section => {
    ...
    section.columns.forEach(colId => {
      const column = editor.state.columns[colId];
      if (!column) return;

      console.log('  → colonna', colId);

      editor.addColumn(section.id, colId, { fromLoad: true });

      // WIDGET
      column.widgets.forEach(wid => {
        const widget = editor.state.widgets[wid];
        if (!widget) return;

        console.log('    → widget', wid);

        editor.addWidgetToColumn(colId, widget, { fromLoad: true });
      });
    });
  });
...
editor.addSection = function (forcedId = null, options = {}) {
  ...
  // crea colonna default solo se NON load
  if (!options.fromLoad) {
...
editor.addColumn = function (sectionId, forcedId = null, options = {}) {
  ...
  const columnId = forcedId ?? ('col-' + Date.now());
...
$.getJSON('/FB-STD/editor/load_layout.php?tema=blog&page=home', {
  ...
  .done(function(data) {
    ...
    if (typeof data === 'string') {
      console.warn("JSON era stringa, parsing...");
      data = JSON.parse(data);
    }

    if (data.empty) {
      console.warn("Pagina vuota");
      editor.resetEditor();
      return;
    }
    ...
    console.log("Layout caricato OK");
  })
...
if (!isset($data['widgets'])) $data['widgets'] = new stdClass();

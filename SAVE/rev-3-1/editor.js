...
editor.renderWidgetPreview = function (widget) {
  ...
  switch (widget.type) {
    ...
    case 'bottone':
      return `<button><a href="${widget.props.url}" class="widget-bottone"style="text-align: ${widget.props.align}">${widget.props.text}</a></button>`;
    case 'image':
...
editor.getDefaultProps = function (type) {
  ...
  switch (type) {
    ...
    case "space":
      return { height: 1 };  
    case "bottone":
      return { text: "CERCA" , url: "#" , align: "center"};
    case "image":
...
editor.addColumn = function (sectionId, forcedId = null, options = {}) {
  ...
  if (!editor.state.columns[columnId]) {
    editor.state.columns[columnId] = {
      ...
      sectionId: sectionId,
      widgets: [],
      props: { background: '#fefefe', padding: '0px' }
    };
  }

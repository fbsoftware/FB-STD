...
// inserimento
    case 'nuovo':
     $btx = new bottoni_str_par('£titolo','£tab','write_£tab.php',array('nuovo','ritorno'));
          $btx->btn();
echo	"<fieldset>";
     $ins = new DB_ins('£tab','£prog');
     $f1 = new input(array($ins->insert(),'£prog',3,'Progressivo','Per ordinamento','i'));
          $f1->field();
     $ts = new DB_tip_i('stato','£stat','','Stato','Attivo/sospeso');
          $ts->select();
	//-----------------------------------------------------------------
     $f3 = new input(array('','ecod',20,'Codice','£tooltip','ia'));
          $f3->field();
     $f4 = new input(array('','edes',20,'Descrizione','£tooltip','i'));
          $f4->field();
     $f3 = new input(array('','etmp',20,' Template','£tooltip','i'));
          $f3->field();
echo "</fieldset>";
	// per textarea
echo	"<fieldset>";
...
// modifica
    case 'modifica':
     $btx = new bottoni_str_par('£titolo','£tab','write_£tab.php',array('modifica','ritorno'));
     $btx->btn();
	...
	require('fields_£tab.php');
     ...
     $f1 = new input(array($£prog,'£prog',3,'Progressivo','Per ordinamento','i'));
          $f1->field();
     $ts = new DB_tip_i('stato','£stat',$£stat,'Stato','Attivo/sospeso');
          $ts->select();
	//-----------------------------------------------------------------
...
// cancellazione
    case 'cancella' :
$btg = new bottoni_str_par('£titolo','£tab','write_£tab.php',array('cancella','ritorno'));
     $btg->btn();
      $sql = "SELECT * FROM `".DB::$pref."£tab`
               WHERE `£id` = $£id  ";
	echo	"<fieldset>";
	...
	require('fields_£tab.php');
      ...
      $f1 = new input(array($£prog,'£prog',3,'Progressivo','','r'));
	 	$f1->field();
      $ts = new input(array($£stat,'£stat',1,'Stato','','r'));
	 	$ts->field();
	//-----------------------------------------------------------------

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // disabilitiamo il pulsante per evitare doppie richieste
    ...
    document.querySelector('#lid').value = data.lid ?? '';
    document.querySelector('#lpage').value = data.lpage ?? '';
    document.querySelector('#ldesc').value = data.ldesc ?? '';
    document.querySelector('#lstat').value = data.lstat ?? '';
    document.querySelector('#ltmp').value = data.ltmp ?? '';
    document.querySelector('#lcod').value = data.lcod ?? '';
    document.querySelector('#ltipo').value = data.ltipo ?? '';
    document.querySelector('#linclude').value = data.linclude ?? '';
}
        const li = e.target.closest(".canvas-item");
         console.log("li selezionato:", li);
        if (!li) return;

        const lid = li.dataset.lid;
        console.log("lid selezionato:", lid);
        if (!lid) return;

        console.log("Ho selezionato lid:", lid);

        // Dettagli (gestione robusta del JSON)
        ...
        .then(text => {
            if (!text || text.trim() === "") {
                console.warn("get-details.php ha restituito testo vuoto");
                return;
            }
            try {
        ...
        .catch(err => {
            console.error("Errore fetch get-details.php:", err);
        });
        ...
        .catch(err => {
            console.error("Errore fetch preview-widget.php:", err);
        });

...
case 'artslide':
               $arg = new DB_sel_l('asl','dprog','','dcod','lcod','dstat','dcod','Codice','Articolo in slide');
          		$arg->select_label(); 
               $f4 = new input(array('artslide.php','lrequire',50,'Programma','Programma da requirere','r')); 
          		$f4->field();    
			break;
case 'arttab':
               $arg = new DB_sel_l('asl','dprog','','dcod','lcod','dstat','dcod','Codice','Articolo in tab');
          		$arg->select_label();       
               $f4 = new input(array('arttab.php','lrequire',50,'Programma','Programma da requirere','r'));    
          		$f4->field();    
			break;
case 'artacc':
               $arg = new DB_sel_l('asl','dprog','','dcod','lcod','dstat','dcod','Codice','Articolo in accordion');
          		$arg->select_label();       
               $f4 = new input(array('artacc.php','lrequire',50,'Programma','Programma da requirere','r'));   
          		$f4->field();    
			break;
case 'artsingle':  // da verificare, altrimenti usare -article-
               $arg = new DB_sel_l('art','aprog','','atit','lcod','astat','atit','Titolo articolo','Articolo semplice');
          		$arg->select_label();       
               $f4 = new input(array('artsingle.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'article':
               $arg = new DB_sel_l('art','aprog','','atit','lcod','astat','atit','Titolo articolo','Articolo semplice');
          		$arg->select_label();       
               $f4 = new input(array('article.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'artimg':
               $arg = new DB_sel_l('aim','iprog','','icod','lcod','istat','icod','Codice','Articolo con immagine');
          		$arg->select_label();       
               $f4 = new input(array('artimg.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'artcol':
               $arg = new DB_sel_l('arc','hprog','','hcod','lcod','hstat','hcod','Codice','Articolo in colonne');
          		$arg->select_label();       
               $f4 = new input(array('artcol.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'glyph':
               $arg = new DB_sel_l('gly','gprog','','gcod','lcod','gstat','gcod','Codice','Modulo con glifi');
          		$arg->select_label();       
               $f4 = new input(array('glyph.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'promo':
               $arg = new DB_sel_l('prm','oprog','','ocod','lcod','ostat','ocod','Codice','Modulo promo');
          		$arg->select_label();       
               $f4 = new input(array('promo.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'portfolio':
               $arg = new DB_sel_l('por','pprog','','pcod','lcod','pstat','pcod','Codice','Modulo portfolio');
          		$arg->select_label();       
               $f4 = new input(array('portfolio.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'slide':
               $arg = new DB_sel_l('sld','slprog','','slcod','lcod','slstat','slcod','Codice','Modulo slide di immagini');
          		$arg->select_label();       
               $f4 = new input(array('slide.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
			break;
case 'header':
     		$f1 = new input(array('','lcod',30,'Codice','Header con navigatore','i'));
          		$f1->field();         
               $f4 = new input(array('header.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
               break;
case 'footer':
               $arg = new DB_sel_l('foo','fprog','','fcod','lcod','fstat','fcod','Codice','Modulo footer');
          		$arg->select_label();       
               $f4 = new input(array('footer.php','lrequire',50,'Programma','Programma da requirere','r'));           
          		$f4->field();    
               break;
case 'contatti':
